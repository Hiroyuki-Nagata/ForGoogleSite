...
EVT_MENU(wxID_ForceScroll, HtmlWindowTest::OnForceScroll)
...
wxFrame(NULL, wxID_ANY, title, wxDefaultPosition, wxSize(640, 480)) {
     ...
     fileMenu->Append(wxID_ForceScroll, wxT("&スクロール\tAlt-S"), wxT("ウィンドウを強制的にスクロールさせます"));
...
void HtmlWindowTest::OnForceScroll(wxCommandEvent& event) {

     // スクロールさせるサイズを入力させる
     long ret = wxGetNumberFromUser(wxT("スクロールさせる値"),
				    wxT("スクロールさせる値"), 
				    wxT("スクロールさせる値"),
				    0,
				    -100,
     ...
     if (ret == -1) {
	  wxMessageBox(wxT("スクロール値は -100 ~ 100までの数値を入力してください"),
	               wxT("ウィンドウのスクロール"), wxICON_ERROR);
	  return;
     }

     int x, y = static_cast<int>(ret);
     htmlWin->ForceScrollWindow(x, y);
}
